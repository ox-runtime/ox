...
bool DetectGraphicsBinding(const void* next, void** outBinding) {
    while (next) {
        ...
        if (header->type == XR_TYPE_GRAPHICS_BINDING_VULKAN_KHR) {
            ...
            LOG_DEBUG("DetectGraphicsBinding: Vulkan graphics binding detected");
            return true;
        }
        next = header->next;
    }
...
XRAPI_ATTR XrResult XRAPI_CALL xrGetVulkanInstanceExtensionsKHR(XrInstance instance, XrSystemId systemId,
    ...
    if (bufferCapacityInput >= len && buffer) {
        strncpy(buffer, extensions, bufferCapacityInput);
...
XRAPI_ATTR XrResult XRAPI_CALL xrGetVulkanGraphicsDeviceKHR(XrInstance instance, XrSystemId systemId,
    ...
    if (selectedDevice == VK_NULL_HANDLE) {
        LOG_ERROR("xrGetVulkanGraphicsDeviceKHR: Failed to select physical device");
    ...
    *vkPhysicalDevice = selectedDevice;
    LOG_INFO(("xrGetVulkanGraphicsDeviceKHR: Selected device: " + std::string(props.deviceName)).c_str());
    return XR_SUCCESS;
}
...
XRAPI_ATTR XrResult XRAPI_CALL xrGetVulkanGraphicsDevice2KHR(XrInstance instance,
    ...
    if (selectedDevice == VK_NULL_HANDLE) {
        LOG_ERROR("xrGetVulkanGraphicsDevice2KHR: Failed to select physical device");
        return XR_ERROR_RUNTIME_FAILURE;
    }
    ...
    vkGetPhysicalDeviceProperties(selectedDevice, &props);
    *vkPhysicalDevice = selectedDevice;
    LOG_INFO(("xrGetVulkanGraphicsDevice2KHR: Selected device: " + std::string(props.deviceName)).c_str());
    return XR_SUCCESS;
}
